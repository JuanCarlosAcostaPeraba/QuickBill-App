...
En lugar de recurrir a Cloud Functions, la app genera el PDF de la factura localmente a través del servicio \texttt{InvoicePDFBuilder}. El proceso se resume a continuación; el código completo puede consultarse en el repositorio público.

\begin{enumerate}
  \item \textbf{Crear el lienzo A4}\newline
  Se instancia un \texttt{UIGraphicsPDFRenderer} con tamaño A4 y metadatos del documento:
...
let renderer = UIGraphicsPDFRenderer(bounds: bounds, format: format)
  ...
  Dentro del bloque \texttt{renderer.pdfData \{ ctx in …\}}, se escribe el título y los datos de empresa y cliente:
  \begin{lstlisting}[language=swift, basicstyle=\ttfamily\small]]
ctx.beginPage()
...
drawBlock(label: "Bill to", lines: [clientName])
  ...
  Se imprime la cabecera y, para cada elemento de \texttt{products}, se dibujan descripción, cantidad, precio unitario y total.
...
amountRow("TOTAL",     invoice.totalAmount, bold: true)
  ...
  \item \textbf{Guardar en \texttt{Documents}}\newline
  El PDF resultante se escribe como \texttt{Invoice-<id>.pdf} en el sandbox de la app y se devuelve la URL para compartir o previsualizar.
\end{enumerate}

Este enfoque permite al usuario generar la factura al instante, sin incurrir en costes adicionales. Para la numeración se reutiliza el \texttt{invoice.id} otorgado por Firestore, garantizando unicidad sin contadores globales.
